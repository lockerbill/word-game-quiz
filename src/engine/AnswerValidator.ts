...
import { getAnswersForCategory } from '../data/answers';
// @ts-ignore
...
export function validateAnswer(
  ...
  if (!trimmed.toUpperCase().startsWith(letter.toUpperCase())) {
    return { valid: false, confidence: 0, matchedAnswer: null, reason: 'wrong_letter' };
  }
  ...
  if (exactMatch) {
    return { valid: true, confidence: 1.0, matchedAnswer: exactMatch, reason: 'exact_match' };
  }
  ...
  // If we don't have answers for this category, be lenient - accept if starts with right letter
  if (knownAnswers.length === 0) {
    return {
      valid: true,
      confidence: 0.5,
